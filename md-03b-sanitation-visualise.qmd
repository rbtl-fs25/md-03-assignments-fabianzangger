...
```{r}
sanitation_cuba_2000_2020_except_sm <- sanitation %>% 
  filter(iso3 == "CUB",
         year == 2000 | year == 2020,
...
```{r}
sanitation_cuba_2000_2020_except_sm %>% 
  count(year)
```
...
```{r}
ggplot(data = sanitation_cuba_2000_2020_except_sm,
...
2.  What would you want to change?
    ...
    It might be more legible if the largest parts were at the bottom.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    There is an Overlap: "safely managed sanitation services" are part of "basic sanitation services" and it would add up to over 100%
...
2.  What do you observe when you look at the code and plot?
    ...
    Sanitation is generally improved in urban areas compared to rural areas.
